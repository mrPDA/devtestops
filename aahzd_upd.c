...
/***************************************************************************
   ...
   aahzd.c - a 'Demo Daemon'

   Based on the 'Hello World' public domain demon written by David Gillies <daggillies@yahoo.com> Sep 2003
...
int main(int argc,char *argv[])
{
	...
	/**************************************************************/
	/* Here is some code added to manage daemon                   */
	...
	if (argc > 1)
        {
		int fd;
		ssize_t len; // Исправлено на тип ssize_t.V2568
 		pid_t pid;
		char pid_buf[16];
		...
		if ((fd = open(gLockFilePath, O_RDONLY)) < 0)
		{
			...
			exit(EXIT_FAILURE);	//warn	V2509
		}
		len = read(fd, pid_buf, 16);
        pid_buf[len] = 0;

        // Используем функцию 'strtol' для безопасного преобразования строки в long int. V2508 The function with the 'atoi' name should not be used.
        char* endptr;
        long int pid_long = strtol(pid_buf, &endptr, 10);
        if (*endptr != '\0')
        {
            // Обработка ошибки преобразования, если в строке есть некорректные символы.
            perror("Invalid PID value in the lock file.");
            exit(EXIT_FAILURE);
        }

        pid = (pid_t)pid_long; // Приводим long int к pid_t.
